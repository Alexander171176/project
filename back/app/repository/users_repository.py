...
class UsersRepository(BaseRepo):  # определение класса UsersRepository, наследующегося от класса BaseRepo
    ...
    async def find_by_nick_name(
        ...
        query = select(Users).where(
            Users.nick_name == nick_name)  # формирование запроса select с фильтром по имени пользователя
    ...
    async def find_by_email(email: str):  # определение статического метода find_by_email, принимающего строку email
        query = select(Users).where(
            Users.email == email)  # формирование запроса select с фильтром по адресу электронной почты
        return (await db.execute(query)).scalar_one_or_none()  # выполнение запроса и возврат результата
