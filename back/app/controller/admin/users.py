...
async def get_user(current_user: Users = Depends(JWTRepo().decode)):
    return current_user
...
async def requires(role: str):
    async def _requires(current_user: Users = Depends(JWTRepo().decode), role: Role = Depends(get_role_by_user)):
...
@app.get("/users")
async def get_users(current_user: Users = Depends(get_user), role: Role = Depends(get_role_by_user)):
...
async def get_user_by_id(user_id: str, current_user: Users = Depends(get_user), role: Role = Depends(get_role_by_user)):
